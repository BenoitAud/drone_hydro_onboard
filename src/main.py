...
timeout = timedelta(minutes=duree)  # Assuming `duree` is in minutes
...
while (datetime.now() - time_init) < timeout and i_failure < 6:
    ...
    try:
        ...
        chemin_success = "file_" + datetime.now().strftime("%Y-%m-%d_%H%M%S") + ".txt"
    ...
    #     print("water temperature is : %f C" %temp_c)
